...
<html lang="en" class="h-100">
  <head>
    <meta charset="utf-8">
        <title>PARK-iN | your Bookings</title>
...
<body id="home" onload="myFunction()" data-spy="scroll" data-target="#navbar-wd" data-offset="98" class=" h-100 text-center text-white bg-dark">
      <!-- LOADER -->
      <!-- <div id="loading"></div> -->
      <!-- END LOADER -->
...
<div class="cover-container d-flex w-100 h-100 p-3 mx-auto flex-column " style="max-height: 130px;min-height: 130px;">
  <header class="mb-4">
    <div>
      ...
      <div class="txt">
        <h1><b>Your Booking History</b></h1>
      </div>
...
{
  echo '<td scope="col" colspan="15">NO BOOKINGS TILL NOW</td>';
} 
...
?>
      ...
      <td scope="col"><?php echo $row['bid']; ?></td>
      <td scope="col"><?php echo $row['time']; ?></td>
      <td scope="col"><?php echo $pslot['npslot']; ?></td>
      ...
      <?php 
        ...
        if($pslot['slotstatus'] !="Active"){
         ?>
      <td class="column6"><a class="btn btn-danger" href="extend.php?uid=<?php echo $row['bid']; ?>" role="button">Extend</a></td>
   <?php 
         }
         else{
          ?> <td class="column6"><i class="fa fa-close"></i></td>
          <?php
         }
       }
    else{ ?>
   ...
   <?php }
    ...
    if($bal<0){
      ...
      <?php 
        if($row['status'] !="Exit"){?>
        <td class="column6"><a class="btn btn-danger" href="user_fine_payment.php?fine_bid=<?php echo $row['bid']; ?>&ps_id=<?php echo $row['ps_id']; ?>" role="button">
         Pay  ₹<?php echo $bal_amt; ?>/-</a></td>
        <?php }
         if($row['status'] =="Exit"){?>
         <td class="column5"><i class="fa fa-close"></i></td>
      ...
      <!-- <td class="column6"><a class="btn btn-danger" href="user_fine_payment.php?fine_bid=<?php echo $row['bid']; ?>&ps_id=<?php echo $row['ps_id']; ?>" role="button">Pay  ₹<?php echo $bal_amt; ?>/-</a></td> -->
        ...
        <td class="column6"><a class="btn btn-danger" onclick="return confirm('Are you sure you are leaving PARK-iN?');" href="?fbid=<?php echo $row['bid']; ?>&ps_id=<?php echo $row['ps_id']; ?>" role="button">Finish</a></td>
<!--      
